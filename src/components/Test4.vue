<template>
  <div class="table-wrap coltype" style="width: 50%">
    <input type="text" v-model="search" class="form-control m-0">
    <table class="table ui-accordion">
      ...
      <tbody>
        <template v-for="row in data.tempRows">
          <tr class="head" :key="row.order">
            <td>{{ row.order }}</td>
            <td>{{ row.type }}</td>
            <td>{{ row.question }}</td>
          </tr>
...
export default {
  ...
  mounted () {
    ...
    this.data.rows = this.data.tempRows = [...jsonData.rows];
  },
  ...
  watch: {
    search: function (newValue, oldValue) {
      this.data.tempRows = this.getSearchResult()
    }
  },
